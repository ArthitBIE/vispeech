...
- **Heuristic scoring**: no ML/model — rule-based approximations of the original MediaPipe visual + Web Speech audio pipeline.
  ...
  - Visual: mouth-open deviation from an ideal percentage per Thai viseme group (`idealMouthOpen` map, taxonomy from `seed.sql`); banded differences (≤10/≤25/≤40 → 90/75/55/40).
...
- Depends on: `SCORING_WEIGHTS` from `@/lib/constants`; viseme-group taxonomy assumed from DB `seed.sql` (`words.viseme_group` values: ริมฝีปากปิด, ปากเปิดกว้าง, ปากห่อกลม, ฟันแตะริมฝีปาก, ปากเปิดกลาง, ทักทาย, ตัวเลข).
